...
class MultiBenchDataModule(LightningDataModule):
    ...
    def __init__(self, dataset: str,
        ...
        self.dataset_kwargs = kwargs # For MOSI, kwargs=[modalities="vision, text", task, **cfg[dataset]["kwargs"]="drop+noise"]
...
class MultiBenchSSL(MultiBench):
    ...
    def __init__(self, dataset: str, split: str, **dataset_kwargs):
        aug_kwargs = MultiBenchAugmentations.parse_kwargs(dataset_kwargs)
        super().__init__(dataset, split, **dataset_kwargs)
        self.augment = MultiBenchAugmentations(**aug_kwargs)# For mosi, augumentation=drop+noise and task=classification
        
        # Parse specific augmentation configuration
        self.use_specific_augmentations = dataset_kwargs.get('use_specific_augmentations', False)
        
        # Create specific augmentations for deterministic control
        # You can customize these augmentations as needed:
        aug1_type = dataset_kwargs.get('aug1_type', 'drop')      # Default: drop for aug1
        aug2_type = dataset_kwargs.get('aug2_type', 'noise')     # Default: noise for aug2
        aug1_prob = dataset_kwargs.get('aug1_prob', 1.0)         # Default: always apply
        aug2_prob = dataset_kwargs.get('aug2_prob', 1.0)         # Default: always apply
    ...
    def __getitem__(self, i):
        ...
        # Check if specific augmentations are configured
        
        if self.use_specific_augmentations:
            # Option 1: Use specific augmentations - deterministic control
            aug1 = self.aug1_specific(X)  # Always drop
            aug2 = self.aug2_specific(X)  # Always noise
        else:
            # Option 2: Use original random augmentations (default behavior)
            aug1 = self.augment(X) # augment all modalities together
            aug2 = self.augment(X)
